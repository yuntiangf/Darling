 ...
 * 
 */
public class FindTopIndicator extends LinearLayout {

	private static final String TAG = "TopIndicator";
//	private int[] mDrawableIds = new int[] {R.drawable.find_recommend,R.drawable.find_fashion,
//					R.drawable.find_category};
	private List<CheckedTextView> mCheckedList = new ArrayList<CheckedTextView>();
	private List<View> mViewList = new ArrayList<View>();
	// 顶部菜单的文字数组
	private CharSequence[] mLabels = new CharSequence[] { "专属推荐", "流行元素", "类别"};
	...
	public FindTopIndicator(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public FindTopIndicator(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public FindTopIndicator(Context context) {
		super(context);
		init(context);
	}
