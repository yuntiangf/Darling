...
public class ViewFlow extends AdapterView<Adapter> {
	...
	//noted by mushrrom 2015.11.30
	//mPager.requestDisallowInterceptTouchEvent(true);是为了解决viewflow左右滑动与最外层viewpager的冲突
	//mScrollView.requestDisallowInterceptTouchEvent(!mGestureDetector.onTouchEvent(ev));
	//是为了解决viewflow左右滑动时造成的微小上下滑动与外层scrollview的冲突,通过判断手势移动的位置落差，判断事件是否传递给scrollview
	@Override
	...
	class YScrollDetector extends SimpleOnGestureListener{
		...
		public boolean onScroll(MotionEvent e1,MotionEvent e2,float distanceX,float distanceY){
			/**
			 * if we're scrolling moew closer to x direction,return false
			 */
